...
void ScrewPackager::connect_to_classificators(const int total,
  ...
  for (size_t i = 0; i < total; i++) {
    this->classificators.push_back(ScrewClassifier(file_classif[i], &(this->classified)));
  }
}
  ...
  while (finish == false || !this->classified.empty()) {
    ...
    if (it == this->screws_unpackaged.end()) {
      ...
      if (this->config.has_type(block.type)) {
        for (size_t j = 0; j < block.quantity; j++) {
          int m = it->second.make_package(block.quantity, block.width, this->config.get_limit(block.type), this->config.get_type(block.type));
          if (m > 0)
            std::cout << "Paquete listo: " << this->config.get_limit(block.type) << " tornillos de tipo "
                  << this->config.get_type(block.type) << " (mediana: " << m << ")\n";
        }
      } else {
